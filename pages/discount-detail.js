...
export default class DiscountDetail extends Component<Props> {
    state = {
        content: null
    };
    ...
    async fetchContent() {
        ...
        return new Promise((resolve, reject) => {
            rest.POST('Discount/detail', `dis_id=${id}`).then((response) => {
            ...
            return(
                <ScrollView style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                    <View style={{display: 'flex', flexDirection: 'column'}}>
                        <View>
                            <Image source={{uri: this.state.content.data.cover, height: 270, width: '100%'}} />
                        </View>
                        ...
                        <View
                            ...
                            <Text>{this.state.content.data.label}</Text>
